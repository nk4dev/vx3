...
function localWebViewBuilder({ blocknum, balance }) {
    ...
    return `<html>
        <head>
            <title>XNV Local Web View</title>
        </head>
        <style>
            * { 
                padding: 0; 
...
export default function localServer(options?: Partial<ServerOptions>) {
            ...
            if (debug) {
                ...
                bn.then(
                    (blockNumber) => {
                        ...
                        res.end(localWebViewBuilder({ blocknum: blocknum, balance }));
                    }
                ).catch(() => {
                    console.error('Error fetching block number');
                    res.end(localWebViewBuilder({ blocknum: 0, balance: 0 }));
                });
            } else {
    ...
    server.listen(portNumber, host, () => {
        if (debug) {
            ...
            console.log(`debug view: http://${host}:${portNumber}/debug`);
            if (chains) {
